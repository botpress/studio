...
class FileNavigator extends React.Component<Props, State> {
  ...
  refreshNodes = () => {
    ...
    const icons = {
      readOnly: (
        <Tooltip content={lang.tr('code-editor.navigator.isReadOnly')}>
          <Icon icon="lock" />
        </Tooltip>
      ),
    ...
    const exampleLabel = (
      <Tooltip
        content={
          <span>
            {lang.tr('code-editor.navigator.codeSamples')}
            <br /> {lang.tr('code-editor.navigator.codeSamples2')}
            <br /> <br /> {lang.tr('code-editor.navigator.cannotBeEdited')}
          </span>
        }
        hoverOpenDelay={500}
        ...
        ContextMenu.show(
          <Menu>
            <MenuItem
              ...
              icon="duplicate"
              text={lang.tr('code-editor.navigator.copyExample')}
        ...
        ContextMenu.show(
          <Menu>
            ...
            <MenuItem
              id="btn-duplicateCurrent"
              icon="duplicate"
              text={lang.tr('code-editor.navigator.copyExampleToHooks')}
      ...
      <Menu>
        {canMove ? (
          <MenuItem
            id="btn-move"
            icon="edit"
            text={lang.tr('code-editor.navigator.renameMove')}
  ...
  render() {
    if (!this.state.nodes.length) {
      return <div className={style.padding}>{lang.tr('code-editor.navigator.noFilesFound')}</div>
    }
