...
export default class Testing extends React.Component<Props, State> {
  ...
  startRecording = async () => {
    try {
      ...
      await this.axios.post('/testing/startRecording', { userId: chatUserId })
  ...
  loadScenarios = async () => {
    const { data } = await this.axios.get<{ scenarios: Scenario[]; status: { running: boolean } }>('/testing/scenarios')
  ...
  deleteAllScenarios = async () => {
    ...
    if (shouldDelete) {
      await this.axios.post('/testing/deleteAllScenarios')
      await this.loadScenarios()
    }
  }
  ...
  runAllScenarios = async () => {
    ...
    await this.axios.post('/testing/runAll')
  ...
  runSingleScenario = async (scenario: Scenario) => {
    ...
    await this.axios.post('/testing/run', { scenario })

    await this.longPoll()
  }

  deleteSingleScenario = async (scenario: Scenario) => {
    await this.axios.post('/testing/deleteScenario', { name: scenario.name })
  ...
  getElementPreviews = async (scenarios: Scenario[]): Promise<{ [id: string]: string }> => {
    const elementIds = _.chain(scenarios)
      .flatMapDeep((scenario) =>
        scenario.steps.map((interaction) => interaction.botReplies.map((rep) => rep.botResponse))
      )
      .filter(_.isString)
      .uniq()
      .value()

    const { data } = await this.axios.post<Preview[]>('/testing/fetchPreviews', { elementIds })
