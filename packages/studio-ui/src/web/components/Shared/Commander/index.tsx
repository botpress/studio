...
const Commander: FC<CommanderProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const getLabel = (label: string, shortcutType: string) => {
      if (shortcutType === 'goto') {
        return lang.tr('commander.goTo', { destination: label })
      } else if (shortcutType === 'popup') {
        return lang.tr('commander.inNewTab', { destination: label })
      }
      return label
    }
    ...
    return (
      <div className={style.item}>
        <span className={style.categoryWrapper}>
          <span className={cx(style.category, style[category])}>{lang.tr(`commander.category.${category}`)}</span>
        </span>
  ...
  const renderHeader = () => {
    return (
      <div className={style.headerWrapper}>
        <span className={style.item}>{lang.tr('commander.searchCommand')}</span>
        <span className={style.item}>
          <kbd className={style.kbd}>↑↓</kbd> {lang.tr('commander.toNavigate')}
        </span>
        <span className={style.item}>
          <kbd className={style.kbd}>enter</kbd> {lang.tr('commander.toSelect')}
        </span>
        <span className={style.item}>
          <kbd className={style.kbd}>esc</kbd> {lang.tr('commander.toDismiss')}
        </span>
      </div>
    )
  }
  ...
  return (
    <CommandPalette
      ...
      commands={commands}
      trigger={<span />}
      placeholder={lang.tr('commander.typeCommand')}
      resetInputOnClose
      closeOnSelect
      options={options}
