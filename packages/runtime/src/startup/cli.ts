  ...
  .command(
    ...
    async (argv) => {
      process.IS_PRODUCTION =
        argv.production || yn(process.env.BP_PRODUCTION) || (yn(process.env.CLUSTER_ENABLED) as boolean)

      process.AUTO_MIGRATE =
        process.env.AUTO_MIGRATE === undefined ? <boolean>yn(argv.autoMigrate) : <boolean>yn(process.env.AUTO_MIGRATE)
