...
export class NLUClientProvider {
  ...
  public async initialize() {
    ...
    const defaultEndpoint = `http://localhost:${process.NLU_ENDPOINT}`
    this._endpoint = autoStartNLUServer ? defaultEndpoint : nluServerEndpoint
    this._baseClient = new NLUClient({ baseURL: this._endpoint })
  }
  ...
  public async mountBot(botId: string) {
    ...
    const client = new NLUClient({ baseURL: this._endpoint, cloud: botConfig.cloud })
