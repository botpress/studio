...
export const BotEditSchema = Joi.object().keys({
  name: Joi.string()
    ...
    .error(
      () =>
        'Bot name must not contain more than 50 alphanumerical characters. Only those special characters are accepted: (, ), -, _, and spaces.'
    ),
  env: Joi.object().pattern(Joi.string().max(50), Joi.string().max(500)).optional(),
